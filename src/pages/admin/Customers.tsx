import { useStore, formatSom, formatDate } from '@/stores/store';
...
const Customers = () => {
  ...
  const handleAddCustomer = () => {
    ...
    saveCustomersData([...customersData, newCustomer]);
    setAddForm({ name: '', phone: '', createdAt: new Date().toISOString().split('T')[0] });
    setShowAddModal(false);
  };

  // Merge customers from orders and manually added customers
  const allCustomers = Object.values(
    [...orders, ...customersData.map(c => ({ 
      customerName: c.name, 
      ...
      if (o.totalPrice) {
        acc[key].orderCount++;
        acc[key].totalSpent += o.totalPrice;
      }
  ...
  const handleSaveEdit = () => {
    ...
    // Update all orders with this customer's phone
    const updatedOrders = orders.map(o => 
      o.customerPhone === editingCustomer.phone 
  ...
  const handleDelete = (phone: string) => {
    ...
    const updatedOrders = orders.filter(o => o.customerPhone !== phone);
    useStore.setState({ orders: updatedOrders });
    localStorage.setItem('orders', JSON.stringify(updatedOrders));
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-center px-4 py-3 text-muted-foreground font-medium">Buyurtmalar</th>
              <th className="text-right px-4 py-3 text-muted-foreground font-medium">Jami summa</th>
              <th className="text-right px-4 py-3 text-muted-foreground font-medium hidden md:table-cell">Kiritilgan sana</th>
              <th className="text-right px-4 py-3 text-muted-foreground font-medium">Amallar</th>
            </tr>
          </thead>
          <tbody>
            {allCustomers.map((c, i) => (
              <motion.tr key={c.phone} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className="border-b border-border last:border-0 hover:bg-muted/20">
                ...
                <td className="px-4 py-3 text-right font-medium format-som">{formatSom(c.totalSpent)}</td>
            ...
            >
              <div className="w-full max-w-md bg-card rounded-xl card-shadow border border-border overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">Kiritilgan sana</label>
                    <input
                      type="date"
