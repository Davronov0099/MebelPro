...
const CashierQueue = () => {
  ...
  useEffect(() => {
    ...
    if (socket && (user?.role === 'admin' || user?.role === 'kassir')) {
      const handleNewCart = (cart: QueueCart) => {
        console.log('📥 New queue cart received:', cart);
      ...
      const handleRemoveCart = (data: { cartId: string }) => {
        console.log('📥 Remove cart from queue (Admin):', data.cartId);
  ...
  const handleApprove = async (queueId: string) => {
    try {
      // Find the cart before updating
      const cart = pendingCarts.find(q => q.id === queueId);
      
      await updateQueueStatus(queueId, 'sent_to_cashier', 'admin');
      setPendingCarts(prev => prev.filter(q => q.id !== queueId));
      toast.success('Savatga qo\'shildi');
      
      // Auto-redirect to Sales page with cart data
      setTimeout(() => {
        navigate('/admin/sales', { state: { approvedCart: cart } });
      }, 500);
    } catch (error) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-bold text-foreground">Kassa xizmati</h2>
          {newNotification && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
